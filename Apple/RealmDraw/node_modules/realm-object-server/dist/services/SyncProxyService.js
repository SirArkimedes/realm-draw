...
let SyncProxyService = class SyncProxyService {
    ...
    syncMasterChanged(backend, handle) {
        ...
        backend.proxy.on("error", (err) => {
            this.logger.detail(`sync proxy error: ${err.message}`);
        });
    ...
    websocketHandler(req, socket, head) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.logger.debug(`Proxy upgrade: [${socketID}] new connection from ${socket.remoteAddress}:${socket.remotePort}`);
            try {
                const { label, identity, authorization } = yield this.parseTokenAndGetLabel(req, socket, head);
                const backend = yield this.getBackend(label);
                if (!backend.proxy) {
                    throw new errors.realm.ServiceUnavailable();
                }
                socket.identity = identity;
                const end = this.connectionDurationHistogram.startTimer({ identity });
                backend.sockets[socketID] = socket;
                this.activeConnectionsGauge.inc({ identity });
                this.logger.debug(`The Realm Object Server proxy has ${Object.keys(backend.sockets).length} open connections to the sync server`);
                socket.on("close", () => {
    ...
    getBackend(syncLabel) {
        if (!this.backendPromises[syncLabel]) {
            ...
            this.backendPromises[syncLabel] = util_1.timeout(this.server.discovery.waitForService("sync", ["role=master", labelTag]), 1000)
                .then((handle) => {
                const backend = {};
