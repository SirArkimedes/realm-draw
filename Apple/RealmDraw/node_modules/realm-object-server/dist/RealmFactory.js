...
class RealmFactory {
    constructor(params) {
        ...
        this.shutdownServer = params.shutdown.bind(params);
    }
    open(definition) {
                ...
                promise = new RealmPromise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const config = {
                        ...
                        sync: {
                            ...
                            user: Realm.Sync.User.adminUser(this.adminToken, uri.scheme("http").origin()),
                            error: (session, err) => {
                                ...
                                if (err.name === "ClientReset") {
                                    console.error(`Client reset occurred for Realm at path ${definition.remotePath}. Shutting down preemptively.`);
                                    util_1.timeout(this.shutdownServer(), 30000)
                                        .then(() => process.exit(1))
                    ...
                    realm = yield Realm.open(config).progress((a, b) => {
                        ...
                        promise.localPath = realm.path;
                        promise.forceClose = (message) => {
                            oldClose();
                            this.logger.detail(message);
    ...
    forceCloseRealm(remotePath, message, deleteAfterClose = false) {
        return __awaiter(this, void 0, void 0, function* () {
            const promise = this.syncedRealmPromises[remotePath];
            if (promise) {
                yield promise;
                promise.forceClose(message);
                if (deleteAfterClose) {
                    yield util_1.delay(500);
                    const realmFolder = path.dirname(promise.localPath);
                    this.logger.detail(`Deleting RealmFactory file. Remote ${remotePath}. Local: ${realmFolder}`);
                    yield util_1.Promisify(rimraf.bind(rimraf), realmFolder);
