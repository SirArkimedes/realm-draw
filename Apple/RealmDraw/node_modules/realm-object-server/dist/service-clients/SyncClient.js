...
class SyncClient extends ServiceClientBase_1.ServiceClientBase {
    ...
    deleteRealm(realmPath) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return yield this.executeRequest(request, this.authorizationHeader());
        });
    }
    compactRealms() {
        return __awaiter(this, void 0, void 0, function* () {
            const services = yield this.findAllServices();
            if (services.length === 0) {
                throw new errors.realm.ServiceUnavailable();
            }
            const responses = services.map((service) => {
                const route = new URI(`http://${service.address}:${service.port}`)
                    .segment(["api", "compact"])
                    .toString();
                const request = superagent.post(route);
                return this.executeRequest(request, this.authorizationHeader());
            });
            yield Promise.all(responses);
        });
    }
    authorizationHeader() {
        return `Realm-Access-Token version=1 token="${this.adminToken}"`;
    }
}
